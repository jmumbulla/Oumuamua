...
library(base)
library(cubature)
###########The density profile and potential of the bulge
#Hernquist spheroid potential
###calculate the potential  
phib<-function(R,z,parameters){
  potb<- -parameters$G*parameters$Mb/sqrt(R^2+z^2+parameters$bb^2)
  return(potb)
}
Potc <- function(r,parameters){
  G <- parameters$G
  Mc1 <- parameters$Mc1
  Mc2 <- parameters$Mc2
  rc1 <- parameters$rc1
  rc2 <- parameters$rc2
  potc <- -G*Mc1/sqrt(r^2+rc1^2)-G*Mc2/sqrt(r^2+rc2^2)
  return(potc)
}
###The density profile is also analytical
denb<- function(R,z,parameters){
       Mb<- parameters$Mb
       bb<- parameters$bb
       G<-  parameters$G
       rhob<- (3*Mb/(4*pi*bb^3))*(1+(R^2+z^2)/bb^2)^{-5/2}
       return(rhob)
}

denb.r <- function(r,parameters){
  Mb<- parameters$Mb
  bb<- parameters$bb
  G<-  parameters$G
  rhob<- (3*Mb/(4*pi*bb^3))*(1+r^2/bb^2)^{-5/2}
  return(rhob)
}
############ potential of the halo
#NFW Dark halo potential
phih <-function(R,z,parameters){
        poth<- -parameters$G*parameters$Mh/sqrt(R^2+z^2+parameters$bh^2)
        return(poth)
}

Poth <- function(R,parameters){
  Vh <- parameters$Vh
  r0 <- parameters$r0
  poth<- 1/2*Vh^2*log(R^2+r0^2)
  return(poth)
}
denh<- function(R,z,parameters){
       Mh<- parameters$Mh
       bh<- parameters$bh
       G<-  parameters$G
       rhoh<- (3*Mh/(4*pi*bh^3))*(1+(R^2+z^2)/bh^2)^{-5/2}
       return(rhoh)
}

denh.r<- function(r,parameters){
       Mh<- parameters$Mh
       bh<- parameters$bh
       G<-  parameters$G
       rhoh<- (3*Mh/(4*pi*bh^3))*(1+r^2/bh^2)^{-5/2}
       return(rhoh)
}

#########the density profile and potential of the stellar disk
#Stellar disk potential according to Miyamoto & Nagai 1975
phid<- function(R,z,parameters){
        potd<- -parameters$G*parameters$Md/sqrt(R^2+(parameters$ad+sqrt(z^2+parameters$bd^2))^2)
        return(potd)
}
Potd <- function(R,z,parameters){
  G <- parameters$G
  Md1 <- parameters$Md1
  Md2 <- parameters$Md2
  Md3 <- parameters$Md3
  ...
  ad3 <- parameters$ad3
  b <- parameters$b
  potd1 <- -G*Md1/sqrt(R^2+(ad1+sqrt(z^2+b^2))^2)
  potd2 <- -G*Md2/sqrt(R^2+(ad2+sqrt(z^2+b^2))^2)
  potd3 <- -G*Md3/sqrt(R^2+(ad3+sqrt(z^2+b^2))^2)
  potd <- potd1+potd2+potd3
  return(potd)
}
#density profile
dend<- function(R,z,parameters){
       M<- parameters$Md
       G<- parameters$G
       a<- parameters$ad
       b<- parameters$bd
       rhod<- (b^2*M/(4*pi))*(a*R^2+(a+3*sqrt(z^2+b^2))*(a+sqrt(z^2+b^2))^2)/((R^2+(a+sqrt(z^2+b^2))^2)^(5/2)*(z^2+b^2)^(3/2))   
       return(rhod)
}

dend.array <- function(x, parameters){
  dend(x[1],x[2],parameters)
}

####In Gardner (2010), his potential include a gas component
Potg <- function(R,z,parameters){
  G <- parameters$G
  Mg1 <- parameters$Mg1
  Mg2 <- parameters$Mg2
  Mg3 <- parameters$Mg3
  ...
  ad3 <- parameters$ad3
  bg <- parameters$bg
  potg1 <- -G*Mg1/sqrt(R^2+(ad1 + sqrt(z^2+bg^2))^2)
  potg2 <- -G*Mg2/sqrt(R^2+(ad2 + sqrt(z^2+bg^2))^2)
  potg3 <- -G*Mg3/sqrt(R^2+(ad3 + sqrt(z^2+bg^2))^2)
  potg <- potg1+potg2+potg3
  return(potg)
}
...
dena <- function(R,z,state,arm.par){
        ...
        Rs <- arm.par$Rs
        H <- arm.par$H
        rhoa <- rho0*exp(-(R-R0)/Rs)/(cosh(z/H))^2
        return(rhoa)}

dens <- function(time,R,phi,z,arm.par,par.model){
#       G <- par.model$G
#       dV.dr <- numerical.derivative(function(R,phi){phis(time,R,phi,state,arm.par)},R,phi)
#       dV.dphi <- numerical.derivative(function(phi,R){phis(time,R,phi,state,arm.par)},phi,R)
#       dV.dr2 <- numerical.derivative(function(R,phi){R*dV.dr},R,phi)/R
#       dV.dphi2 <- numerical.derivative(function(phi,R){dV.dphi},phi,R)/R^2
#       rhos <- (dV.dr2+dV.dphi2)/(4*pi*G)
        t <- time
        R0 <- arm.par$R0
        alpha <- arm.par$alpha
        m <- arm.par$m
        Rmin <- arm.par$Rmin
        Rmax <- arm.par$Rmin+arm.par$extent
        Omegap <- arm.par$Vp
        extent <- arm.par$extent
        chi <- m*(alpha*log(R/Rmin)-phi+Omegap*t)
#        if((R>Rmin)&(R<Rmin+extent)){
#          chi <- m*(alpha*log(R/Rmin)-phi+Omegap*t)}else{
#          chi <- pi/2}
        rhos <- dena(R,z,state,arm.par)*cos(chi)
        return(rhos)
}

###the axisymmetric galactic potential amplitude
phia <- function(time, R, phi, z, pars){
        t <- time
        R0 <- pars$R0
        alpha <- pars$alpha
        rho0 <- pars$rho0
        ...
        m <- pars$m
        Rmin <- pars$Rmin
        Rmax <- pars$Rmin+pars$extent
        Omegap <- pars$Vp
        G <- pars$G
        chi <- m*(alpha*log(R/Rmin)-phi+Omegap*t)
        K1 <- m/(R*sin(atan(1/alpha)))
        beta1 <- K1*H*(1+0.4*K1*H)
        D1 <- (1+K1*H+0.3*(K1*H)^2)/(1+0.3*K1*H)
        pota <- -4*pi*pars$G*H*rho0*exp(-(R-R0)/Rs)*cos(chi)*(cosh(K1*z/beta1))^-beta1/(K1*D1)
        return(pota)
       }

###galactic density without arm
dent <- function(R,z,parameters){
        denb(R,z,parameters)+denh(R,z,parameters)+dend(R,z,parameters)
}

#Define differentiating function
numerical.derivative <- function(f,x,y,h = 1e-8)
{
   # test something
   (f(x + h,y) - f(x,y)) / h}

numerical.derivative.1D <- function(f,x,h=1e-8){
   (f(x+h) -f(x))/h}

deriv.array <- function(y,x){
	    leny <- length(y)
	    lenx <- length(x)
	    d0 <- (y[2]-y[1])/(x[2]-x[1])
	    d1 <- (y[leny]-y[leny-1])/(x[lenx]-x[lenx-1])
	    x1 <- x[-c(lenx-1,lenx)]
	    x2 <- x[-c(1,2)]
	    y1 <- y[-c(leny-1,leny)]
	    y2 <- y[-c(1,2)]
...
laplace.1D <- function(f,x,h=1e-6){
   (f(x+h)-2*f(x)+f(x-h))/h^2}
laplace.cylind.2D <- function(f,r,z,h=1e-6){
          (f(r - h,z) + f(r+h,z) +f(r,z+h) + f(r,z-h) - 4*f(r,z)) / h^2 +(f(r+h/2,z)-f(r-h/2,z))/h/r
}
###creat a color vector for showing 2d surface
zeroone <- function(x) {
  x.r <- range(x, na.rm=TRUE)
  (x - x.r[1])/(x.r[2]-x.r[1])
}
###########the total galactic potential
phit<- function(R,z,parameters){
  phib(R,z,parameters)+phih(R,z,parameters)+phid(R,z,parameters)
}
Pott <- function(R,z,parameters){
  Potc(R,parameters)+Poth(R,parameters)+Potd(R,z,parameters)+Potg(R,z,parameters)
}
dent<- function(R,z,parameters){
  denb(R,z,parameters)+denh(R,z,parameters)+dend(R,z,parameters)
}
Dent <- function(R,z,parameters){
  laplace.cylind.2D(function(R,z) Pott(R,z,parameters),R,z)
}
Denc <- function(R,parameters){
  laplace.1D(function(R) R*Potc(R,parameters),R)/R
}
Denh <- function(R,parameters){
  laplace.1D(function(R) R*Poth(R,parameters),R)/R
}
Dend <- function(R,z,parameters){
  laplace.cylind.2D(function(R,z) Potd(R,z,parameters),R,z)
}
Deng <- function(R,z,parameters){
  laplace.cylind.2D(function(R,z) Potg(R,z,parameters),R,z)
}

##############the rotation curve
vc.b <- function(R, parameters){
  G <- parameters$G
  z <- 0
  Fb.R <- parameters$G*parameters$Mb/(R^2+z^2+parameters$bb^2)^1.5*R
  Fr <- Fb.R
  return(sqrt(R*Fr))
}
vc.h <- function(R, parameters){
  G <- parameters$G
  z <- 0
  Fh.R <- parameters$G*parameters$Mh/(R^2+z^2+parameters$bh^2)^1.5*R
  Fr <- Fh.R
  return(sqrt(R*Fr))
}
vc.d <- function(R, parameters){
  G <- parameters$G
  z <- 0
  Fd.R <- parameters$G*parameters$Md/(R^2+(parameters$ad+sqrt(z^2+parameters$bd^2))^2)^1.5*R
  Fr <- Fd.R
  return(sqrt(R*Fr))
}
calc.vp <- function(x,y,z,vx,vy,vz,parameters){
    kpcmyr2auyr <- 1e3*206265/1e6
    R <- sqrt(x^2+y^2)#kpc
    vc <- Vcirc(R,parameters)/kpcmyr2auyr#kpc/myr
    phi <- xy2phi(x,y)
    vr <- vx*cos(phi)+vy*sin(phi)
    vt <- -vx*sin(phi)+vy*cos(phi)
    u <- -vr
    v <- -(vt+vc)
    w <- vz
    return(cbind(u,v,w))
...
Vcirc <- function(R, parameters){
#  G <- parameters$G
    kpc2au <- 1e3*206265
    R <- R*kpc2au#au
    G = mu = 39.4753#4*pi^2, leading to velocity in units of au/yr
    bb <- parameters$bb*kpc2au
    bh <- parameters$bh*kpc2au
    bd <- parameters$bd*kpc2au
    ad <- parameters$ad*kpc2au
    z <- 0
    Fb.R <- G*parameters$Mb/(R^2+z^2+bb^2)^1.5*R
    Fh.R <- G*parameters$Mh/(R^2+z^2+bh^2)^1.5*R
    Fd.R <- G*parameters$Md/(R^2+(ad+sqrt(z^2+bd^2))^2)^1.5*R
    Fr <- Fb.R+Fh.R+Fd.R
    return(sqrt(R*Fr))#in unit of au/yr
}
#######################################
#####statistics of arm/plane crossings
#######################################
mypeaks<- function(x){
...
###dmax [kpc]; tmax [Myr]; rs [kpc]; vs [kpc/Myr]
    ...
    vc0 <- -Vcirc(Rs,par.gal)/kpcmyr2auyr#kpc/myr
...
hel2gal<- function(Dc, b, l, Rs,Zs){
    ...
    return(list(phi=phig,R=Rg,X=Xg,Y=Yg, Z=Zg))
}
...
bs.step <- function(f,time,y0,Dt,tol=1e-7){
##t are forward times
    if(all(time<=0)){
        sign <- -1
    }
    ...
    ic <- matrix(state[k,],ncol=1)
    for(j in 1:Nstep){
        inds <- which(tt>=(j-1)*Dt & tt<=(j*Dt))
        pp <- bulirsch_stoer(f=f, t=sign*(tt[inds]-min(tt[inds])), y0=ic,tol=tol)
